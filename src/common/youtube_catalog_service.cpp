...
// Finds the "give me the next page" token anywhere in an InnerTube response.
//
// YouTube buries it in a different place per surface (search puts it at the end
// of the item section, browse feeds inside the grid, continuation responses
// inside appendContinuationItemsAction), so walking the whole tree for the
// first one is both simpler and more robust than encoding every path.
std::string extract_continuation_token(const json& node) {
    std::string token;
    const auto walk = [&](const auto& self, const json& current) -> void {
        if (!token.empty() || current.is_null()) {
            return;
        }

        if (current.is_object()) {
            if (current.contains("continuationItemRenderer")
                && current.at("continuationItemRenderer").is_object()) {
                token = get_string(
                    current.at("continuationItemRenderer")
            ...
            // Older responses still use the pre-2021 shape.
            if (current.contains("nextContinuationData")
                && current.at("nextContinuationData").is_object()) {
                token = get_string(current.at("nextContinuationData"), "continuation");
                if (!token.empty()) {
                    return;
                }
            }
...
std::optional<HomeFeed> YouTubeCatalogService::fetch_home_feed(
    ...
    feed.continuation = results.continuation;
...
std::optional<HomeFeed> YouTubeCatalogService::fetch_authenticated_browse_feed(
    ...
    feed.continuation.source = ContinuationSource::BROWSE;
    feed.continuation.token = extract_continuation_token(root);
...
SearchResults YouTubeCatalogService::fetch_search_results(
    ...
    results.continuation.source = ContinuationSource::SEARCH;
    results.continuation.token = extract_continuation_token(root);

    this->cache_stream_details(results.items);

    logf(
        "youtube: search query=%s items=%zu more=%d",
        query.c_str(),
        results.items.size(),
        results.continuation.valid() ? 1 : 0);
    return results;
}
...
std::optional<FeedPage> YouTubeCatalogService::fetch_search_continuation(
    ...
    if (!response.has_value() || response->empty()) {
        this->error_message_ = "다음 검색 페이지 요청 실패";
        return std::nullopt;
    }

    const json root = json::parse(*response, nullptr, false);
    if (root.is_discarded()) {
        this->error_message_ = "다음 검색 페이지 응답 파싱 실패";
        return std::nullopt;
    }

    FeedPage page;
    std::unordered_set<std::string> seen_ids;
    collect_stream_items(root, allow_short_videos, limit, seen_ids, page.items);
    page.continuation.source = ContinuationSource::SEARCH;
    page.continuation.token = extract_continuation_token(root);

    this->cache_stream_details(page.items);
    this->error_message_.clear();
    return page;
}

std::optional<FeedPage> YouTubeCatalogService::fetch_browse_continuation(
    const std::string& token,
    size_t limit,
    bool allow_short_videos) const {
    json payload = {
    ...
    // Browse continuations only ever come from signed-in feeds here (home
    // recommendations and subscriptions), so they need the same SAPISIDHASH
    // headers the first page was fetched with.
    std::string auth_error;
    ...
    const auto response = this->client_->post(kBrowseApiUrl, payload.dump(), headers);
    if (!response.has_value() || response->empty()) {
        this->error_message_ = "다음 피드 페이지 요청 실패";
        return std::nullopt;
    }

    const json root = json::parse(*response, nullptr, false);
    if (root.is_discarded()) {
        this->error_message_ = "다음 피드 페이지 응답 파싱 실패";
        return std::nullopt;
    }
    ...
    std::unordered_set<std::string> seen_ids;
    collect_stream_items(root, allow_short_videos, limit, seen_ids, page.items);
    page.continuation.source = ContinuationSource::BROWSE;
    page.continuation.token = extract_continuation_token(root);
...
std::optional<FeedPage> YouTubeCatalogService::get_more_items(
    ...
    const AppSettings settings = SettingsStore::instance().settings();
    const bool allow_short_videos = !settings.hide_short_videos;

    std::optional<FeedPage> page;
    switch (continuation.source) {
        case ContinuationSource::SEARCH:
            page = this->fetch_search_continuation(continuation.token, 24, allow_short_videos);
            break;
        case ContinuationSource::BROWSE:
            page = this->fetch_browse_continuation(continuation.token, 24, allow_short_videos);
            break;
        case ContinuationSource::NONE:
            return std::nullopt;
    }
    ...
    logf(
        "youtube: continuation source=%d items=%zu more=%d",
        static_cast<int>(continuation.source),
        page->items.size(),
        page->continuation.valid() ? 1 : 0);
    return page;
}
