...
public:
    ...
    // Fetches the page that follows a feed or search result. Feeds carry their
    // own continuation, so the caller only has to hand back what the previous
    // page returned.
    std::optional<FeedPage> get_more_items(const Continuation& continuation) const;
...
private:
    ...
    SearchResults fetch_search_results(
        ...
        size_t limit,
        bool allow_short_videos) const;
    std::optional<FeedPage> fetch_search_continuation(
        const std::string& token,
        size_t limit,
        bool allow_short_videos) const;
    std::optional<FeedPage> fetch_browse_continuation(
        const std::string& token,
        size_t limit,
        bool allow_short_videos) const;
